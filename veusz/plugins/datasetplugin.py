...
class Dataset2D(_DatasetBase):
    ...
    def __init__(self, name, data=[[]], rangex=None, rangey=None,
        ...
        self.name = name
        self.update(
            data=data, rangex=rangex, rangey=rangey,
...
class DatasetPluginHelper:
    ...
    def datasets1d(self):
        ...
        return [
            name for name, ds in self._doc.data.items() if
    ...
    def getDataset(self, name, dimensions=1):
        ...
        if isinstance(ds, datasets.DatasetDateTime):
            ...
            return Dataset1D(
                name, data=ds.data, serr=ds.serr,
...
class DatasetPluginManager:
    """Manage datasets generated by plugin."""

    def __init__(self, plugin, doc, fields, raiseerrors=False):
        ...
        self.setupDatasets(raiseerrors=raiseerrors)
    ...
    def setupDatasets(self, raiseerrors=False):
        ...
        for ds in self.datasets:
            if not ds.name.strip() and raiseerrors:
                raise DatasetPluginException(_("Output dataset name is empty"))
    ...
    def saveToFile(self, fileobj):
        ...
        names = {}
        for ds, dsname in zip( self.veuszdatasets, self.datasetnames ):
            try:
...
class MultiplyDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Multiply a dataset by a factor. Error bars are also scaled.')
...
class InterleaveDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Join datasets, interleaving element by element.\n'
        'Error bars are merged.')
...
class ChopDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Chop out a section of a dataset. Give starting '
...
class PartsDatasetPlugin(DatasetPlugin):
    ...
    description_full = _(
        'Split dataset into equal-size parts. '
...
class ThinDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Select data points at intervals from dataset '
...
class MeanDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Compute mean of multiple datasets to create a single dataset.')
...
class AddDatasetsPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Add datasets together to make a single dataset. '
...
class SubtractDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Subtract two datasets. Combined error bars are also calculated.')
...
class DivideDatasetsPlugin(_OneOutputDatasetPlugin):
    ...
    description_short = _(
        'Compute ratio or fractional difference'
        ' between two datasets')
    description_full = _(
        'Divide or compute fractional difference'
        ' between two datasets')
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldDataset('ds_in2', _('Input dataset 2')),
            field.FieldBool(
                'frac', _('Compute fractional difference'), default=False),
            field.FieldDataset('ds_out', _('Output dataset name')),
        ]
...
class ExtremesDatasetPlugin(DatasetPlugin):
    ...
    description_full = _(
        'Compute extreme values of input datasets. Creates '
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldDataset('ds_errorbar', _(
...
class DemultiplexPlugin(DatasetPlugin):
    ...
    description_full = _(
        'Split dataset into multiple datasets on an '
...
class FilterDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Filter a dataset using an expression, '
        'e.g. "x>10" or "(x>1) & (y<2)"')
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldBool(
                'replacenan', _(
                    'Replace excluded points by NaN\n'
    ...
    def updateDatasets(self, fields, helper):
        ...
        try:
            ...
            raise DatasetPluginException(
                _("Error filtering dataset: '%s')") % str(e))
...
class MovingAveragePlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Compute moving average for regularly spaced data.'
    ...
    def __init__(self):
        ...
        self.fields = [
            field.FieldDataset('ds_in', _('Input dataset')),
            field.FieldInt(
                'width', _('Points either side of point to average'),
                default=1, minval=0),
            field.FieldBool(
                'weighterrors', _('Weight by error bars'),
                default=True),
            field.FieldDataset('ds_out', _('Output dataset')),
        ]
...
class LinearInterpolatePlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        "Compute linear interpolation of x,y data.\n"
...
class ReBinXYPlugin(DatasetPlugin):
    ...
    description_full = (
        'Given dataset Y (and optionally X), for every N '
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldInt(
                'binsize', _('Bin size (N)'), minval=1, default=2),
            field.FieldCombo(
                'mode', _('Mode of binning'),
                items=('sum', 'average'),
                editable=False),
...
class Histogram2D(DatasetPlugin):
    ...
    description_full = _(
        'Given two 1D datasets, compute a 2D histogram. '
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldInt(
                'binsx', _('Number of bins for dataset x'),
                default=10, minval=2),
            ...
            field.FieldInt(
                'binsy', _('Number of bins for dataset y'),
                default=10, minval=2),
...
class ConvertNumbersToText(DatasetPlugin):
    ...
    description_full = _(
        'Given a 1D numeric dataset, create a text dataset '
